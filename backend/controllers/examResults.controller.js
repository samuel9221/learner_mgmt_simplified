...
const _recomputeFinalResult = async (client, learnerId, subjectId, streamId, termId) => {
  ...
  const midRow = sessions.find(s => s.exam_type === 'mid_term');
  const endRow = sessions.find(s => s.exam_type === 'end_of_term');

  const midScore = midRow?.score ?? null;   // raw score out of 100
  const endScore = endRow?.score ?? null;   // raw score out of 100
...
const create = async (req, res) => {
  ...
  try {
    ...
    const { learner_id, subject_id, stream_id, exam_session_id, score, is_absent } = req.body;
    ...
    const result = await client.query(
      `INSERT INTO exam_results
         (learner_id, subject_id, stream_id, exam_session_id, score, is_absent, teacher_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       ON CONFLICT (learner_id, subject_id, exam_session_id)
       ...
       RETURNING *`,
      [learner_id, subject_id, stream_id, exam_session_id, is_absent ? null : score, is_absent ?? false, teacher_id]
    );
